...
PastSounds = new Meteor.Collection('pastSounds');
POSTREQUESTS = new Meteor.Collection('postrequests');
YoutubeVideos = new Meteor.Collection('youtubeVideos');
DashboardImages = new Meteor.Collection('dashboardImages');
...
Router.route('azure', {
    ...
    .get(function(){
        ...
        Meteor.call('pushSound', "DRACO", "GET REQUEST DONE", lang, true);

        this.response.end('Hello, get request\n');
    })
    .post(function(){
        var json_data = this.request.body;
        Meteor.call('pushSound', "DRACO", "POST REQUEST DONE", lang, true);
        if (json_data){
            // console.log(json_data);
            POSTREQUESTS.insert(json_data);
        }
        this.response.end('Hello, post request\n');
    });
  ...
  .post(function () {
    ...
    if (command){
        ...
        if (mycommands['commands'][command]){
            ...
            if (mycommand['active'] && json_data['user_name'] !== 'jsavage'){
